...
public class JavaPackage {
    ...
    public JavaPackage(String name, int volatility) {
        ...
        afferentsCount = new HashMap<JavaPackage, Integer>();
        efferentsCount = new HashMap<JavaPackage, Integer>();
    }
    ...
    public void addAfferent(JavaPackage jPackage) {
        if (!jPackage.getName().equals(getName())) {
            ...
            Integer packageCount = afferentsCount.get(jPackage);
            if (packageCount != null) {
            	packageCount++;
            } else {
            	packageCount = 1;
            }
            afferentsCount.put(jPackage, packageCount);
        }
    }
    ...
    public void addEfferent(JavaPackage jPackage) {
        if (!jPackage.getName().equals(getName())) {
        	...
        	Integer packageCount = efferentsCount.get(jPackage);
            if (packageCount != null) {
            	packageCount++;
            } else {
            	packageCount = 1;
            }
            efferentsCount.put(jPackage, packageCount);
        }
    }
    ...
    public void setEfferentsCount(Map<JavaPackage, Integer> efferentsCount) {
		this.efferentsCount = efferentsCount;
	}
    
    public Map<JavaPackage, Integer> getAfferentsCount() {
		return afferentsCount;
	}
    
    public void setAfferentsCount(Map<JavaPackage, Integer> afferentsCount) {
